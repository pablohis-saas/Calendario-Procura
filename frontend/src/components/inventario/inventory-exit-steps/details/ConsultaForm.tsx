      ...
      <CardContent className="space-y-4">
        <div className="flex gap-2 items-end">
          <select value={selected} onChange={e => setSelected(e.target.value)} className="border rounded px-2 py-1">
            ...
            {consultaItems.map(item => <option key={item} value={item}>{item}</option>)}
          </select>
          <input type="number" min={1} max={3} value={cantidad} onChange={e => setCantidad(Number(e.target.value))} className="w-16 border rounded px-2 py-1" />
